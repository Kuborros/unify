...
| Pokémon Trading Card Game Live    | pokemon                          | 2022.3.21f1   | no              | no             | no       |                                               |
| Freedom Planet 2                  | 595500                           | 5.6.3f1       | no              | no             | **yes**  | Needs ``FP2_Data/Plugins/x86_64/libCSteamworks.so + libsteam_api.so``. These can be obtained from ``Steamworks.NET-Standalone_2025.164.1.zip`` package from Steamworks.Net releases. For controller support ``Assembly-CSharp.dll`` needs to be patched[^2]|
